...
//Возвращает x-значение точки пересечения двух линий
...
//Возвращает y-значение точки пересечения двух линий.
...
//Эта функция обрезает все края по отношению к одному краю отсечения области обрезки
void clip(int poly_points[][2], int &poly_size, int x1, int y1, int x2, int y2) {
    int new_points[MAX_POINTS][2], new_poly_size = 0;

    //(ix, iy), (kx, ky) - значения координат точек
    for (int i = 0; i < poly_size; i++) {
        //i и k образуют линию в многоугольнике
        int k = (i+1) % poly_size;
        int ix = poly_points[i][0], iy = poly_points[i][1];
        int kx = poly_points[k][0], ky = poly_points[k][1];
        //Расчет позиции первой точки относительно отрезающей линии
        int i_pos = (x2-x1) * (iy-y1) - (y2-y1) * (ix-x1);
        ////Расчет позиции второй точки относительно отрезающей линии
        int k_pos = (x2-x1) * (ky-y1) - (y2-y1) * (kx-x1);

        //1 Случай: Обе вершины находятся внутри:
        if (i_pos < 0  && k_pos < 0) {
            //Точки не добавляются
        }
        //2 Случай: Первая вершина находится снаружи, а вторая - внутри:
        else if (i_pos >= 0  && k_pos < 0) {
            //Добавляется только точка пересечения с краем
            new_points[new_poly_size][0] = x_intersect(x1, y1, x2, y2, ix, iy, kx, ky);
            new_points[new_poly_size][1] = y_intersect(x1, y1, x2, y2, ix, iy, kx, ky);
            new_poly_size++;
        }
            //3 Случай: Первая вершина находится внутри, а вторая - снаружи:
        else if (i_pos < 0  && k_pos >= 0) {
            //Добавляется точка пересечения с краем и вторая точка
            new_points[new_poly_size][0] = x_intersect(x1, y1, x2, y2, ix, iy, kx, ky);
            new_points[new_poly_size][1] = y_intersect(x1, y1, x2, y2, ix, iy, kx, ky);
            new_poly_size++;
            ...
            //4 Случай: Обе вершины находятся снаружи:
        else {
            //Добавляется только вторая точка
            new_points[new_poly_size][0] = kx;
            new_points[new_poly_size][1] = ky;
            new_poly_size++;
        }
    ...
    for (int i = 0; i < new_poly_size; i++) {
        Point point = { new_points[i][0], new_points[i][1] };
...
//Реализация алгоритма Сазерленда – Ходжмана
void suthHodgClip(int poly_points[][2], int poly_size, int clipper_points[][2], int clipper_size) {
    //i и k - два последовательных индекса
    for (int i = 0; i < clipper_size; i++) {
        int k = (i + 1) % clipper_size;
        //Мы передаем текущий массив вершин, его размер и конечные точки выбранной линии отсечения
